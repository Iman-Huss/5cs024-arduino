...
int emerPush = 0; // int to store the value of emergencyPin
int emergencyPin = 1; // int to store the arduino pin number that emergency button is connected
...
void openValve(){
  for (int i = 0; i<8; i++){
    emerPush = digitalRead(emergencyPin);
    if (emerPush == 0){ // Check if emergency button was pushed
